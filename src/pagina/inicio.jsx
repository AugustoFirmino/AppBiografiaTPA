...
const LoaderOverlay = () => {
  ...
  return (
    ...
    >
      <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-red-600 shadow mb-4 group-hover:scale-105 transition-transform duration-300 bg-gray-100 flex items-center justify-center">
        {imgUrl ? (
          <img src={imgUrl} alt={dir.nome} className="w-full h-full object-cover" />
        ) : (
  ...
  <span className="text-xs text-gray-500 text-center mt-5">
  {dir.biografia.length > 100 
    ? dir.biografia.substring(0, 100) + "..." 
    : dir.biografia}
</span>
